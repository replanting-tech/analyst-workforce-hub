...
export function DashboardOverview() {
  ...
  const avgResponseTime =
    incidents.length > 0
      ? Math.round(
          incidents.reduce((acc, i) => acc + Math.random() * 30 + 5, 0) /
            incidents.length
        )
      : 0;

  // Calculate SLA compliance
  const totalClosedIncidents = incidents.filter(
    (i) => i.status === "closed"
  ).length;
  const slaMetIncidents = incidents.filter(
    (i) => i.status === "closed" && i.sla_status === "met"
  ).length;
  const slaCompliance =
        ...
        <Card className="bg-gradient-to-br from-blue-50 to-blue-100 border-blue-200">
          ...
          <CardContent>
            ...
            <p className="text-xs text-blue-600">
              {availableAnalysts} available, {busyAnalysts} busy
            </p>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Latest incident activities and status
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      {slaDashboard.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Overview of SLA performance across all customers
            </CardDescription>
          </CardHeader>
          <CardContent>
                    ...
                    <div className="flex justify-between text-xs">
                      ...
                      <span
                        className={`font-medium ${
                          item.sla_compliance_percentage >= 95
                            ? "text-green-600"
                            : item.sla_compliance_percentage >= 90
                            ? "text-orange-600"
                            : "text-red-600"
                        }`}
                      >
